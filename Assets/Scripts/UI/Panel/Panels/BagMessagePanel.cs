...
public class BagMessagePanel : BasePanel
{
    public RectTransform bk; //背景
    public TextMeshProUGUI itemName; //物品名字
    private Dictionary<string, AttributeInfo> itemAttributes; //物品属性信息
    ...
    /// <param name="anchoredPos">所在位置</param>
    public void Init(ItemSO data, Vector2 anchoredPos)
    {
        itemAttributes = new Dictionary<string, AttributeInfo>();
        //Vector2 screenPoint = Camera.main.WorldToScreenPoint(worldPos);
        //RectTransformUtility.ScreenPointToLocalPointInRectangle(bk,screenPoint,null,out Vector2 localPos);
        bk.anchoredPosition = anchoredPos;
        SetInfo(data);
    }
    ...
    public void SetInfo(ItemSO data)
    {
        ...
        //更新背景高度
        bk.sizeDelta = new Vector2(bk.sizeDelta.x, nowHeight + 50);
    }
    ...
    public void CreateAttributeInfo(string name, string info)
    {
        ...
        if (!itemAttributes.ContainsKey(name))
        {
            ...
            attributeObj.transform.SetParent(bk, false);
            //设置位置
